...
if __name__ == '__main__':
    ...
    for results_dir in reversed(results_dirs):
        ...
        try:
            ...
            plt.figure(fignum)
            plt.fill_between(range(len(signal)), signal, facecolor=display.GOOD_COLOR_CYCLE[0], alpha=1.0) # alpha=0.5
            ...
            display.save_fig_to_workspace()
            results_figure_name = os.path.join(results_dir, 'anomaly_rating.png')
            display.savefig(results_figure_name)
            ...
            plt.clf()
            plt.fill_between(range(len(thresholded_anom_results)), thresholded_anom_results, facecolor=display.GOOD_COLOR_CYCLE[1], alpha=1.0, label='anomalous: {:.4g}%'.format(100.0 * np.sum(thresholded_anom_results > 0) / len(thresholded_anom_results)))
            plt.legend()
            plt.ylim([0, y_max])
            plt.title(title)
            print('Saving figure to {}'.format(results_figure_name.replace('rating', 'rating_thresholded')))
            display.savefig(results_figure_name.replace('rating', 'rating_thresholded'))
            ...
            if videonum_as_str.find(video_id) != -1 and lambd == 10:
                ...
                anomalousness_to_save += [anomalousness]
                pars_to_save += [pars]
                desination_dir = '/home/allie/workspace/etc/{}_results'.format(video_id)
